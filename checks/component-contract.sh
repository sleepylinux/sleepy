...
jq -e --slurpfile reviewed "$manifest" '
  ...
  .homeManager.shellUnit == "sleepy-shell.service" and
  .homeManager.shellExecStart ==
    [(.packages["sleepy-shell"].path + "/bin/sleepy-shell")] and
...
mkdir -p "$runtime/home" "$runtime/config"
