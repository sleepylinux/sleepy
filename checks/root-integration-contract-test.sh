...
assert_contains .github/workflows/check.yml 'nix flake check --no-write-lock-file --all-systems --no-build --show-trace'
assert_contains .github/workflows/check.yml 'nix build --no-write-lock-file .#checks.x86_64-linux.sleepy-desktop-qml --no-link -L'
