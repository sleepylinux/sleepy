...
require_literal checks/default.nix 'sleepyDesktopQml = inputs.sleepy-desktop.checks.${pkgs.stdenv.hostPlatform.system}.qml.overrideAttrs'
require_literal checks/default.nix 'nativeBuildInputs = (oldAttrs.nativeBuildInputs or []) ++ [pkgs.git];'
require_literal checks/default.nix 'dbusSessionForChecks = pkgs.writeShellScriptBin "dbus-run-session"'
require_literal checks/default.nix '--config-file=${pkgs.dbus.out}/share/dbus-1/session.conf'
require_literal checks/default.nix 'mktemp -d /tmp/sleepy-desktop-check.XXXXXX'
require_literal checks/default.nix '${pkgs.coreutils}/bin/chmod -R u+w "$writable_source"'
require_literal checks/default.nix '${pkgs.coreutils}/bin/ln -s ${inputs.sleepy-sdk} "$check_tmp/sleepy-sdk"'
require_literal checks/default.nix 'SLEEPY_TEST_QPA_PLATFORM:-$QT_QPA_PLATFORM'
require_literal checks/default.nix 'SLEEPY_TEST_WAYLAND_COMPOSITOR= SLEEPY_TEST_SWAY= bash tests/run.sh'
require_literal checks/default.nix '"cd ${inputs.sleepy-desktop}\n"'
...
require_literal flake.nix 'baselineActivationPackage = baselineChecks.activationPackage;'
require_literal flake.nix 'baselineSessionPackage = baselineChecks.sessionPackage;'
require_literal lib/baseline-check-packages.nix 'baseline.homeConfigurations."lazy@sleepy-vm".extendModules'
require_literal lib/baseline-check-packages.nix 'baseline.packages.${system}.sleepy-session.overrideAttrs'
